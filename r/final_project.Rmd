...
# Data-preprocessing (continuous)

Response is highly skewed right. We will be looking at its log-transformation `y`, which has a more normally-distributed shape.
```{r}
dfii %>% 
  select(y) %>%
  tibble::rowid_to_column() %>% 
...
For xw_02 and xw_03 which are highly skewed to the right and left respectively, we are going to center and scale the data during training and testing process.
...
# i) Categorical linear model
```{r, pt02_sec01, eval=TRUE}
mod01 <- lm(y ~ region + customer, data = dfii)
mod01 %>% readr::write_rds('mod01.rds')
```

# ii) Continuous linear model
```{r, pt02_sec02, eval=TRUE}
mod02 <- lm(y ~ (.), data = dfii[c(1,c(3:36))])
mod02 %>% readr::write_rds('mod02.rds')
```

# iii) All categorical and continuous model (linear additive)
```{r, pt02_sec03, eval=TRUE}
mod03 <- lm(y ~ (.), data = dfii)
mod03 %>% readr::write_rds('mod03.rds')
```

# iv) Region with continuous model
```{r, pt02_sec04, eval=TRUE}
mod04 <- lm(y ~ (.), data = dfii[c(1,c(3:36))])
mod04 %>% readr::write_rds('mod04.rds')
```

# v) Customer with continuous model
```{r, pt02_sec05, eval=TRUE}
mod05 <- lm(y ~ (.), data = dfii[c(2:36)])
mod05 %>% write_rds('mod05.rds')
```

# vi) All pairwise interaction between continous inputs
```{r, pt02_sec06, eval=TRUE}
mod06 <- lm(y ~ (.)^2, data = dfii[c(3:36)])
mod06 %>% write_rds('mod06.rds')
```

# vii) Basis of choice 1: [BASIS NAME]
```{r, pt02_sec07_1, eval=FALSE}

```

# viii) Basis of choice 2: [BASIS NAME]
```{r, pt02_sec07_2, eval=FALSE}
```

# ix) Basis of choice 3: [BASIS NAME]
```{r, pt02_sec07_3, eval=FALSE}
```
